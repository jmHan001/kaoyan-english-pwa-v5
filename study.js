...
import{rate,dueWords,wrongWords,slayWord,migrateMemoryModel}from'./review-manager.js?v=6.0.0';
...
let pool,queue=[],index=0,currentOptions=[],recentDistractors=[],answered=false,reviewMode=false,quizMode=false,quizScore=0,quizResults=[],quizSaved=false,questionType='recognition',questionStartedAt=0;
...
function setPoolCompleted(word,done){
  if(!word)return;
  pool=getPool();
  ...
  if(done){next.todayDoneDate=today();if(old.slain)next.slainCompletedToday=true}else if(next.todayDoneDate===today())delete next.todayDoneDate;
...
function renderLearningDetails(word){
  ...
  $('#meaning').textContent=coreTranslation(word);$('#fullMeaning').textContent=cleanTranslation(word);$('#syllable').textContent=`拆着记：${chunks(word.word)}`;
...
function renderQuestion(word,record){
  ...
  const recall=questionType==='recall',context=questionType==='context',deferChoices=!recall&&!context&&(record.recognitionCount>0||memoryStage(record)==='relearning');
  $('#meaningRecall').classList.toggle('hidden',!recall);$('#choices').classList.toggle('hidden',recall||deferChoices);$('#contextQuestion').classList.toggle('hidden',!context);$('#showChoices').classList.toggle('hidden',!deferChoices);
  $('#recallAnswer').value='';$('#recallAnswer').disabled=false;$('#submitRecall').disabled=false;
  if(context){const sentence=examplesFor(word)[0]?.sentence||'';$('#contextSentence').innerHTML=makeInteractiveText(sentence,{highlight:[word.word]});$('#recallCue').textContent='语境验证：判断句中这个词的核心含义';renderChoices(word)}
...
function render(){
  ...
  $('#memoryPack').classList.add('hidden');$('#recallCue').classList.remove('hidden');$('#choiceFeedback').classList.add('hidden');$('#nextWord').classList.add('hidden');$('#reveal').classList.remove('hidden');
...
function finishAnswer(correct,selectedIndex=-1,meta={}){
  ...
  if(!reviewOnly&&!quizMode&&!correct)queue.splice(Math.min(index+4,queue.length),0,word.word);
  if(!reviewOnly&&mode==='today'){if(correct)setPoolCompleted(word.word,true);else setPoolCompleted(word.word,false)}
  ...
  $('#phonetic').textContent=`美 /${word.us||'暂无'}/　英 /${word.uk||'暂无'}/`;$('#memoryPack').classList.remove('hidden');$('#recallCue').classList.add('hidden');$('#showChoices').classList.add('hidden');$('#reveal').classList.add('hidden');$('#nextWord').classList.remove('hidden');
  const feedback=$('#choiceFeedback'),match=meta.match;if(questionType==='recall'&&!correct&&match?.accepted?.length)feedback.textContent=`${feedbackFor(record,false,reviewOnly)} 可接受核心义：${match.accepted.slice(0,4).join('、')}`;else feedback.textContent=feedbackFor(record,correct,reviewOnly);
