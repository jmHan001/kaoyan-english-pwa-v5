import{getSyncConfig,saveSyncConfig,clearSyncConfig,isConfigured,createSyncId,syncNow,exportSyncFile,importSyncFile,startAutoSync}from'./cloud-sync.js?v=5.6.1';
...
if(location.hash.startsWith('#pair=')){try{const pair=decodePair(location.hash.slice(6));if(pair.v===1&&pair.syncId&&pair.passphrase){fields.syncId.value=pair.syncId;fields.passphrase.value=pair.passphrase;history.replaceState(null,'',location.pathname);status('','已读取电脑配对信息','点击“保存并同步”，即可打开本机持续互通。')}}catch{toast('配对链接无效，请重新从电脑复制')}}else setReady();
startAutoSync((kind,error)=>{if(kind==='busy')status('busy','正在自动同步','正在检查另一台设备的新记录。');else if(kind==='ready')setReady();else status('error','自动同步失败',error.message)});
