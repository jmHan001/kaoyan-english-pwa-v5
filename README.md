...
学习页使用四选一中文释义测试。只有连续答对 3 次才标记为已掌握；答错或点击“不会”会清零连续记录，并让原词在本轮稍后回炉。今日学习池生成后保持固定，切换模式或每日数量只影响下一个学习日；新增、替换和补满单词必须由用户在学习池编辑页主动操作。页面会持续显示今日已答对数量、总任务量和剩余数量，只有答对才计入今日完成。
