...
let pool,index=0,queue=[],reviewMode=false,currentOptions=[],answered=false,quiz=null,quizOptions=[],recentDistractors=[],studyOpen=false;
...
function renderWrongList(){const wrap=$('#wrongList');if(!wrap)return;const words=wrongWords(),state=getState();$('#wrongMeta').textContent=`全部错词 ${words.length} 个`;wrap.innerHTML=words.length?words.slice(0,80).map(word=>{const item=findWord(word),record=state.records[word]||{};return`<div class="row"><div><strong>${word}</strong><small>${item?.translation||'当前词库未收录'} · 错 ${record.errors||0} 次 · 连续正确 ${record.correctStreak||0}/3</small></div><div class="row-actions"><button class="iconbtn" data-speak-word="${word}" title="播放发音">▶</button></div></div>`}).join(''):'<div class="empty">还没有错词。答错后的词会自动进入这里。</div>'}
...
function startWrongReview(count){const words=shuffle(wrongWords());const selected=count==='all'?words:words.slice(0,Math.max(1,Number(count)||10));$('#wrongModal').classList.add('hidden');startWordReview(selected,selected.length?`错词回炉：从全部 ${words.length} 个错词中随机打开 ${selected.length} 个。`:'暂无错词。答错后的词会自动进入错词回炉。','错词回炉')}
function startFavoriteReview(){const words=shuffle(favoriteWords());$('#favoriteModal').classList.add('hidden');startWordReview(words,words.length?`重点词表：本次打开 ${words.length} 个收藏词。`:'重点词表为空。背词时点右上角星标加入。','重点词表')}
function renderLearnedList(){const wrap=$('#learnedList');if(!wrap)return;const words=learnedWords(),state=getState();$('#learnedMeta').textContent=`已背 ${words.length} 个单词`;wrap.innerHTML=words.length?words.slice(0,160).map(word=>{const item=findWord(word),record=state.records[word]||{};const status=record.tailStage?'已掌握':record.errors?'错过':record.drawn?'已抽到':'已记录';return`<div class="row"><div><strong>${word}</strong><small>${item?.translation||'当前词库未收录'} · ${status} · 连续正确 ${record.correctStreak||0}/3</small></div><div class="row-actions"><button class="iconbtn" data-speak-word="${word}" title="播放发音">▶</button><button class="iconbtn" data-toggle-favorite="${word}" title="收藏重点">${record.favorite?'★':'☆'}</button></div></div>`}).join(''):'<div class="empty">还没有已背记录。完成一次学习池后会出现在这里。</div>'}
function openLearnedModal(){renderLearnedList();$('#learnedModal').classList.remove('hidden')}
function startLearnedReview(){const words=shuffle(learnedWords());$('#learnedModal').classList.add('hidden');startWordReview(words,words.length?`已背重温：本次打开 ${words.length} 个已背单词。`:'还没有已背单词。先完成今日学习池。','已背重温')}
...
function startTodayWords(){pool=getPool();queue=pool.items.filter(x=>!pool.completed.includes(x));reviewMode=false;index=0;$('#afterTask').classList.add('hidden');$('#done').classList.add('hidden');if(!queue.length){closeStudyWindow('今日任务已完成，可以加背或去做小测。');return}openStudyWindow('今日背词');$('#reviewNotice').textContent=`今日背词：还有 ${queue.length} 个词。`;$('#reviewNotice').classList.remove('hidden');render()}
...
function chunks(word){const parts=word.match(/[^aeiouy]*[aeiouy]+(?:[^aeiouy](?=[^aeiouy]*[aeiouy])|[^aeiouy]*$)/gi);return parts?.length>1?parts.join(' · '):word}function current(){return findWord(queue[index])}function render(){if(!studyOpen){$('#card').classList.add('hidden');$('#afterTask').classList.add('hidden');updateFavoriteButton();renderAcceptance(renderTaskProgress());return}const w=current();
answered=false;
...
function finishChoice(correct,selectedIndex=-1){if(answered)return;const w=current();if(!w)return;answered=true;const record=rate(w.word,correct?3:1,w.source);if(!correct)queue.splice(Math.min(index+4,queue.length),0,w.word);if(correct&&!reviewMode)setPoolCompleted(w.word,true);if(!correct)setPoolCompleted(w.word,false);const buttons=[...document.querySelectorAll('.choice-option')];buttons.forEach((button,i)=>{button.disabled=true;if(currentOptions[i]?.word===w.word)button.classList.add('correct');else if(i===selectedIndex)button.classList.add('wrong')});$('#phonetic').textContent=`美 /${w.us||'暂无'}/　英 /${w.uk||'暂无'}/`;$('#memoryPack').classList.remove('hidden');$('#recallCue').classList.add('hidden');$('#reveal').classList.add('hidden');$('#nextWord').classList.remove('hidden');const feedback=$('#choiceFeedback');feedback.textContent=correct?(record.correctStreak>=3?'回答正确，已连续正确 3 次，标记为已掌握。':`回答正确，连续正确 ${record.correctStreak}/3。`):'本次未计入今日完成，连续正确已清零，并加入本轮回炉。';feedback.className=`choice-feedback ${correct?'correct':'wrong'}`;$('#memoryBadge').textContent=record.tailStage?'已连续正确 3 次':`连续正确 ${record.correctStreak||0}/3`;$('#memoryBadge').classList.toggle('tail',!!record.tailStage);renderStats()}
...
$('#closeStudy').onclick=()=>closeStudyWindow('已退出背词窗口，回到主页。');
