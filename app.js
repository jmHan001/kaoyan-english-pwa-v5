...
import{rate,dueWords,wrongWords,restoreSlainWord,migrateMemoryModel}from'./review-manager.js?v=6.0.0';
import{memoryLabel,memoryStage}from'./memory-engine.js?v=6.0.0';
...
function acceptance(task=renderTaskProgress()){const attempts=quizBook()[today()]?.attempts||[],activeAttempts=attempts.filter(x=>Array.isArray(x.kinds)&&x.kinds.length===x.total&&x.kinds.every(kind=>kind==='recall'||kind==='context')),best=activeAttempts.reduce((best,x)=>!best||x.score/x.total>best.score/best.total?x:best,null),needQuiz=Math.min(10,Math.max(1,task.total||0)),taskOk=task.total>0&&task.done>=task.total,quizDone=best&&best.total>=needQuiz,quizRate=best?Math.round(best.score/best.total*100):0,quizOk=quizDone&&quizRate>=80;let status='待完成',tone='pending',advice='先完成今日学习池，再做一次无选项小测。';if(taskOk&&!quizDone){status='待小测';advice=`今日新词已完成，还需要做 ${needQuiz} 题左右的无选项小测。`}if(taskOk&&quizDone&&!quizOk){status='需加练';tone='warn';advice=`小测正确率 ${quizRate}%，低于 80%。先复习错词，再测一组。`}if(taskOk&&quizOk){status='合格';tone='pass';advice=`主动回忆小测 ${best.score}/${best.total}，今天算真的背到了。`}return{status,tone,advice,taskOk,quizDone,quizOk,quizRate,best,attempts,needQuiz}}
