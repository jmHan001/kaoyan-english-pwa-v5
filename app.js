...
import{localDateKey,timestampValue}from'./date-utils.js?v=6.0.0';
...
function learnedWords(view='all'){const state=getState();return Object.entries(state.records||{}).filter(([,record])=>{const known=record?.drawn||record?.lastSeen||record?.level||record?.errors||record?.favorite,mastered=memoryStage(record)==='mastered';return known&&(view==='all'||(view==='mastered'?mastered:!mastered))}).sort((a,b)=>(Number(b[1].lastSeen)||0)-(Number(a[1].lastSeen)||0)).map(([word])=>word)}
function setFavorite(word,value){if(!word)return;const state=getState(),item=findWord(word),old=state.records[word]||{};state.records[word]={...old,source:item?.source||old.source,drawn:old.drawn||!!item,favorite:!!value,updatedAt:Date.now()};saveState(state);updateFavoriteButton();renderFavoriteList()}
...
function renderLearnedList(){const wrap=$('#learnedList');if(!wrap)return;const words=learnedWords(learnedView),state=getState(),learning=learnedWords('learning').length,mastered=learnedWords('mastered').length;$('#learnedMeta').textContent=`学习中与待验证 ${learning} · 长期掌握 ${mastered}`;document.querySelectorAll('[data-learned-view]').forEach(button=>{const active=button.dataset.learnedView===learnedView;button.classList.toggle('active',active);button.classList.toggle('secondary',!active)});wrap.innerHTML=words.length?words.slice(0,160).map(word=>{const item=findWord(word),record=state.records[word]||{},status=record.slain?'已斩 · 已移出学习计划':memoryLabel(record);return`<div class="row"><div><strong>${word}</strong><small>${item?coreTranslation(item):'当前词库未收录'} · ${status}${record.errors?` · 历史错 ${record.errors} 次`:''}</small></div><div class="row-actions"><button class="iconbtn" data-speak-word="${word}" title="播放发音">▶</button>${record.slain?`<button class="iconbtn" data-restore-slain="${encodeURIComponent(word)}" title="恢复斩词" aria-label="恢复 ${word}">复</button>`:`<button class="iconbtn" data-toggle-favorite="${word}" title="收藏重点">${record.favorite?'★':'☆'}</button>`}</div></div>`}).join(''):`<div class="empty">${learnedView==='mastered'?'还没有通过跨日主动回忆和语境验证的单词。':'当前没有学习中的单词。'}</div>`}
...
function renderTaskProgress(){if(!pool)return{total:0,done:0,recalled:0};repairTodayPool();const total=pool.items.length,completed=new Set(pool.completed||[]),state=getState(),done=pool.items.filter(word=>completed.has(word)||todayDone(word)).length,recalled=pool.items.filter(word=>state.records[word]?.todayRecalledDate===today()).length,remaining=Math.max(0,total-done),finished=total>0&&done===total,progress=$('#todayProgress'),start=$('.today-start');$('#todayCount').textContent=`${done}/${total}`;$('#todayRemaining').textContent=total?(remaining?`还剩 ${remaining} 个单词需要完成学习`:`今日已接触 ${done} 个 · 当场想起 ${recalled} 个`):'学习池为空，请在编辑学习池中添加单词';progress.max=Math.max(1,total);progress.value=done;$('#todayTask').classList.toggle('complete',finished);if(start){start.textContent=finished?'今日已完成':'继续背词';start.classList.toggle('is-complete',finished);start.setAttribute('aria-disabled',String(finished));start.href=finished?'#acceptancePanel':'study.html?mode=today'}$('#restoreTodayTask')?.classList.toggle('hidden',!recoveryMode||!total||done>0);return{total,done,recalled}}
function restoreTodayTask(){pool=getPool();const total=pool.items.length,state=getState(),recent=Object.entries(state.records||{}).filter(([,record])=>timestampValue(record?.lastSeen)>0&&!record?.slain).sort((a,b)=>timestampValue(b[1].lastSeen)-timestampValue(a[1].lastSeen)).slice(0,total).map(([word])=>word).filter(word=>findWord(word));if(recent.length===total)pool.items=recent;pool.completed=[...pool.items];pool.date=today();pool.updatedAt=Date.now();localStorage.setItem('ky5_pool',JSON.stringify(pool));for(const word of pool.items)state.records[word]={...(state.records[word]||{}),drawn:true,source:findWord(word)?.source||state.records[word]?.source,todayDoneDate:today(),todayExposedDate:today()};saveState(state);const task=renderTaskProgress();renderAcceptance(task);renderStats();$('#reviewNotice').textContent=`已从最近学习记录恢复今日接触记录：${task.done}/${task.total}。`;$('#reviewNotice').classList.remove('hidden')}
function renderSyncStatus(status='idle',error=null){const el=$('#syncStatus');if(!el)return;const configured=isConfigured(getSyncConfig()),meta=readJson('ky5_sync_meta',{});el.className=`sync-pill ${configured?'configured':'missing'} ${status}`;if(status==='busy')el.textContent='正在同步';else if(status==='offline')el.textContent='离线保存中 · 联网后补同步';else if(status==='error')el.textContent=`同步异常 · ${String(error?.message||error||'点此处理').slice(0,22)}`;else el.textContent=configured?`已同步 · ${formatTime(meta.lastSyncAt||meta.lastAppliedAt)}`:'未绑定同步';}
...
function acceptance(task=renderTaskProgress()){const attempts=quizBook()[today()]?.attempts||[],activeAttempts=attempts.filter(x=>Array.isArray(x.kinds)&&x.kinds.length===x.total&&x.kinds.every(kind=>kind==='recall'||kind==='context')),best=activeAttempts.reduce((best,x)=>!best||x.score/x.total>best.score/best.total?x:best,null),needQuiz=Math.min(10,Math.max(1,task.total||0)),taskOk=task.total>0&&task.done>=task.total,quizDone=best&&best.total>=needQuiz,quizRate=best?Math.round(best.score/best.total*100):0,quizOk=quizDone&&quizRate>=80;let status='待接触',tone='pending',advice='先完成今天至少 20 个词的学习流程；记不住的会自动回炉。';if(taskOk&&!quizDone){status='已接触 · 待验证';advice=`今天接触 ${task.done} 个，当场想起 ${task.recalled||0} 个。再做 ${needQuiz} 题无选项小测，检查不是靠熟悉感。`}if(taskOk&&quizDone&&!quizOk){status='已完成 · 待巩固';tone='warn';advice=`小测正确率 ${quizRate}%。今天不必死磕，错词已经进入回炉，明天继续验证。`}if(taskOk&&quizOk){status='今日通过';tone='pass';advice=`主动回忆小测 ${best.score}/${best.total}；真正掌握仍以隔天还能想起为准。`}return{status,tone,advice,taskOk,quizDone,quizOk,quizRate,best,attempts,needQuiz}}
function renderAcceptance(task=renderTaskProgress()){const a=acceptance(task),panel=$('#acceptancePanel');if(!panel)return;panel.className=`acceptance-panel ${a.tone}`;panel.innerHTML=`<div><span class="section-kicker">今日验收</span><strong>${a.status}</strong><p>${a.advice}</p></div><div class="acceptance-checks"><span class="${a.taskOk?'ok':'wait'}">今日接触 ${task.done}/${task.total}</span><span class="${task.recalled?'ok':'wait'}">当场想起 ${task.recalled||0}</span><span class="${a.quizDone?'ok':'wait'}">无提示小测 ${a.best?`${a.best.score}/${a.best.total}`:'未测'}</span></div><button class="btn secondary" data-home-action="${a.taskOk?'quiz':'today'}">${a.taskOk?(a.quizOk?'再测一组':'开始验收'):'继续今日背词'}</button>`}
...
function quizCandidates(){pool=getPool();const state=getState();return todayQuizCandidates(pool,state.records,today()).map(word=>findWord(word)).filter(Boolean)}
function startQuiz(){const items=quizCandidates();if(!items.length){$('#reviewNotice').textContent='今日学习池里还没有已背过的单词。先完成几个今日词，再来小测。';$('#reviewNotice').classList.remove('hidden');return}location.href='study.html?mode=quiz&count=10'}
...
migrateMemoryModel();
const savedSettings=getSettings(),s={...savedSettings,daily:Math.max(20,Number(savedSettings.daily)||20)};
if(s.daily!==savedSettings.daily)saveSettings(s);
$('#mode').value=s.mode;
ensureDailyOption(s.daily);
pool=getPool();
if(pool.items.length<s.daily)pool=resize(s.daily);
repairTodayPool();
...
function applyDaily(daily){daily=Math.max(20,Math.min(300,Math.floor(Number(daily)||20)));ensureDailyOption(daily);saveSettings({mode:$('#mode').value,daily});pool=resize(daily);const task=renderTaskProgress();$('#reviewNotice').textContent=`已同步今日任务：至少接触 ${task.total} 个，当前完成 ${task.done} 个。`;$('#reviewNotice').classList.remove('hidden');renderStats();renderAcceptance(task)}
