...
function startTodayWords(){pool=getPool();queue=pool.items.filter(x=>!pool.completed.includes(x));reviewMode=false;index=0;$('#afterTask').classList.add('hidden');$('#done').classList.add('hidden');$('#reviewNotice').textContent=queue.length?`今日背词：还有 ${queue.length} 个词。`:'今日任务已完成，可以加背或去做小测。';$('#reviewNotice').classList.remove('hidden');render()}
